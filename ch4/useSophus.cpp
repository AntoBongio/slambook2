...
int main(int argc, char **argv) {
  ...
  Sophus::SO3d SO3_R(R);              // Sophus::S03d constructed from Rotation
  Sophus::SO3d SO3_q(q);              // from quaternion

  cout << "SO(3) from matrix:\n" << SO3_R.matrix() << endl;
  cout << "SO(3) from quaternion:\n" << SO3_q.matrix() << endl;
  cout << "they are equal" << endl;

  // Use Logaritmic Map to get Lie Algebra
  Vector3d so3 = SO3_R.log();
  cout << "so3 = " << so3.transpose() << endl;
  // ::hat is from vector to skew-symmetric matrix
  cout << "so3 hat=\n" << Sophus::SO3d::hat(so3) << endl;
  // inversely, fro matrix to vector
  ...
  Sophus::SE3d SE3_Rt(R, t);           // Build SE3 from R,t
  Sophus::SE3d SE3_qt(q, t);            // or from q,t
  cout << "SE3 from R,t= \n" << SE3_Rt.matrix() << endl;
  cout << "SE3 from q,t= \n" << SE3_qt.matrix() << endl;
  // Lie Algebra is a 6d vector, we give a typedef
